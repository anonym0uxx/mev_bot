#!/usr/bin/env python
"""c10 gate runner - spec section 8, gates 1-14, as executable checks.
...
CORPUS = "/training/v2/candidate_sft_c10"
...
LEGAL_SIZE = {"SMALL", "MID", "FULL", "NONE", None}
...
C8_REF = {"decision": 85086, "management_replay": 39821,
          "utility_regression": 85086, "utility_reasoning": 10568}
...
def main():
    ...
    for split in args.splits:
        for r in rows(split):
            ...
            # 8
            if fam == "decision":
                if (m.get("size_dimension") or {}).get("size") not in LEGAL_SIZE:
    ...
    G[8] = "PASS" if (lambdas <= ALLOWED_LAM and missing_lam == 0
                      and not bad["8_illegal_size_tier"]) else "FAIL"
    ...
    json.dump(out, open("/training/v2/reports/C10_GATES.json", "w"), indent=1)
