...
// ─── V2 instruction variants ────────────────────────────────────────────
// The on-chain program graduated to V2 entrypoints. Verified against real
// captured instruction bytes (see derive_discriminators.py / verify_discriminator.py);
// each equals sha256("global:<name>")[..8]. Omitting these silently routed
// launches and V2 trades into `unknown_events`.
const PUMP_CREATE_V2: [u8; 8] = [214, 144, 76, 236, 95, 139, 49, 180];
const PUMP_BUY_V2: [u8; 8] = [184, 23, 238, 97, 103, 197, 211, 61];
const PUMP_SELL_V2: [u8; 8] = [93, 246, 130, 60, 231, 233, 64, 178];
const PUMP_MIGRATE_V2: [u8; 8] = [187, 203, 18, 31, 206, 237, 254, 41];
const PUMP_COMPLETE_V2: [u8; 8] = [77, 202, 226, 2, 186, 4, 169, 18];
...
impl Normalizer {
    ...
    pub fn normalize_tx(
        ...
        let mut account_keys_b58: Vec<String> = tx_info
            .transaction
            .as_ref()
        ...
        // pre/post balance arrays are indexed over STATIC keys followed by
        // address-table-lookup keys. Append the loaded keys so index-based
        // lookups are aligned; without this the SOL leg of a swap is
        // unrecoverable from the normalized record.
        if let Some(m) = meta {
            for k in m.loaded_writable_addresses.iter() {
                account_keys_b58.push(b58_encode(k));
            }
            for k in m.loaded_readonly_addresses.iter() {
                account_keys_b58.push(b58_encode(k));
            }
        }
    ...
    fn classify_instruction(
        ...
        let (venue, event_type, trade_side) = if program_id == PUMP_FUN_PROGRAM_ID {
            if disc == PUMP_BUY || disc == PUMP_BUY_V2 {
                ("pumpfun", "buy", Some("buy"))
            } else if disc == PUMP_SELL || disc == PUMP_SELL_V2 {
                ("pumpfun", "sell", Some("sell"))
            } else if disc == PUMP_CREATE || disc == PUMP_CREATE_V2 {
                ("pumpfun", "create", None)
            } else if disc == PUMP_COMPLETE || disc == PUMP_COMPLETE_V2 {
                ("pumpfun", "complete", None)
            } else if disc == PUMP_MIGRATE || disc == PUMP_MIGRATE_V2 {
