...
const sessionTrades = allMevTrades.filter(t => t.entryTimestampMs >= sessionStartMs);
...
// --- All-time MEV stats ---
const mTotal = allMevTrades.length;
const mWins = allMevTrades.filter(l => l.pnlSol > 0).length;
const mPnl = allMevTrades.reduce((s, l) => s + (l.pnlSol || 0), 0);
...
const block1 = [
  ...
  `  WR ${sSesWR}% | PnL ${sSesPnl >= 0 ? '+' : ''}${sSesPnl.toFixed(4)} SOL`,
...
const block2 = [
  `📊 Overall ${mode} P&L`,
  ``,
  `🎯 MEV: ${mTotal} trades | WR ${mWR}% | ${mPnl >= 0 ? '+' : ''}${mPnl.toFixed(4)} SOL`,
  `📈 Scalper: ${sTotal} trades | WR ${sWR}% | ${sPnl >= 0 ? '+' : ''}${sPnl.toFixed(4)} SOL`,
  `💰 Combined: ${combinedPnl >= 0 ? '+' : ''}${combinedPnl.toFixed(4)} SOL`,
].join('\n');
