...
fn inverting_lowers_and_single_application() {
    let base = ScoringInputs { base_score: 500, smart_money_applied: false, smart_money_delta: 0 };
...
fn qualified_raises_unqualified_neutral() {
    let base = ScoringInputs { base_score: 100, smart_money_applied: false, smart_money_delta: 0 };
    let q = apply_smart_money(base.clone(), SmartMoneyClass::Qualified);
    assert!(q.base_score > base.base_score);
    let base2 = ScoringInputs { base_score: 100, smart_money_applied: false, smart_money_delta: 0 };
