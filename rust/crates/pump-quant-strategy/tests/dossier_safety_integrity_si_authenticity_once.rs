...
fn each_mode_touches_only_its_channel() {
    ...
    let edge = apply_authenticity(s.clone(), AuthConfidence { bps: 5000 }, AuthMode::ThroughEdge);
...
fn double_application_rejected() {
    ...
    let twice = apply_authenticity(once.clone(), AuthConfidence { bps: 5000 }, AuthMode::Haircut);
