...
fn impossible_move_is_fabricated() {
    ...
    let flow = FlowWindow { net_inflow: 1, appreciation_bps: 50_000, age_secs: 0 };
    match lpi_score(&flow, 1_000_000, MarketPhase::Late) {
        LpiVerdict::Fabricated { covariate_bps, threshold_margin } => {
...
fn depth_normalization_changes_verdict() {
    let flow = FlowWindow { net_inflow: 100, appreciation_bps: 5_000, age_secs: 0 };
...
fn covariate_decays_with_age() {
    let young = FlowWindow { net_inflow: 1, appreciation_bps: 50_000, age_secs: 0 };
    let old   = FlowWindow { net_inflow: 1, appreciation_bps: 50_000, age_secs: LPI_COVARIATE_HALF_LIFE_SECS };
    let cy = match lpi_score(&young, 1_000_000, MarketPhase::Late) {
        LpiVerdict::Fabricated { covariate_bps, .. } => covariate_bps, _ => panic!() };
