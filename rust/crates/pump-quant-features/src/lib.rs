...
//!
//! This crate implements three operator-mandated pieces of the feature plane,
//! all of them deterministic and free of floating-point arithmetic in every
//! outcome-controlling path (operator 22):
//!
//! * [`timed_feature`] — the [`TimedFeature`](timed_feature::TimedFeature) record and a
//!   **point-in-time-correct** serving store (operator 20). Serving `as_of(T)`
...
//!   flow into time bars or volume bars (operator 21.6). Bars bind back to the
...
//!   (operator 21.6): compression/expansion, breakout-and-retest,
...
//!   gates (operator 6.4, 20, 99).
//! * [`micro`] — an integer AMM order-flow / microstructure feature catalog
//!   (operator 21.7): CVD, delta velocity, order-flow imbalance, VWAP /
...
//!   imagined order book.
//!
//! ## Determinism & purity contract (operator 22)
//!
