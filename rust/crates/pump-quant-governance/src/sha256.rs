//! From-scratch, integer-only SHA-256 (FIPS 180-4).
//!
//! ## Responsibility
//! Provide a deterministic 32-byte digest used as the primitive under the
//! reproducible [`crate::hashing`] guards (§56.3 StrategyHash /
//! EvaluatorReleaseHash, §44 evaluator pinning). Implemented here — rather than
//! pulled from a dependency — so the crate stays self-contained and every gate
//! result is reproducible with no external code.
//!
//! ## §22 / §705 compliance
...
//! the wrap is the specification, not an accident. This is the one place in the
//! crate where wrapping (not checked/saturating) is correct.
//!
//! ## Verification
