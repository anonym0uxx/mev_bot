...
const RUNNER_TAIL_BP: [i64; 3] = [34_000, 46_000, 43_000];
/// A milder bleeder, used only by the shape-robustness sweep.
const MILD_BAD_TAIL_BP: [i64; 3] = [10_500, 9_900, 9_700];
/// A smaller runner, used only by the shape-robustness sweep.
const SMALL_RUNNER_TAIL_BP: [i64; 3] = [18_000, 22_000, 21_000];
/// A larger runner, used only by the shape-robustness sweep.
const HUGE_RUNNER_TAIL_BP: [i64; 3] = [58_000, 82_000, 76_000];
...
/// wallet ladder below.
const SOCIAL_QUALITY: [u64; 7] = [1_200, 1_300, 1_400, 1_500, 1_600, 1_700, 1_800];
...
impl Tape {
    ...
    const fn with_tails(self, runner_tail: [i64; 3], bad_tail: [i64; 3]) -> Self {
        Self {
    ...
    fn path(&self, shape: Shape) -> [i64; 6] {
...
fn markets(tape: Tape) -> Vec<Market> {
...
fn drive(cfg: Config, tape: Tape, mut trace: Option<&mut FlagTrace>) -> Engine {
    let mut cfg = cfg;
...
fn run(base: Config, armed: bool, step_bp: u32, tape: Tape) -> Arm {
    run_traced(base, armed, step_bp, tape, false)
}

fn run_traced(base: Config, armed: bool, step_bp: u32, tape: Tape, trace_flags: bool) -> Arm {
    let mut cfg = base;
