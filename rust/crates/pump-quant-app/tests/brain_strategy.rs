...
fn drive(cfg: Config, decayed: bool) -> Engine {
    ...
    for round in 0..6u64 {
        for m in 0..n {
            ...
            if round == m % 4 {
                if m % 2 == 0 {
                    // The curve's two reserves at THIS moment, from one snapshot:
                    // the price reserve these trades declared and the SOL it
                    // therefore escrows. Re-pin #27: the fixture used to assert a
                    // payout depth ABOVE the whole price reserve.
                    let vsol = REAL_CURVE_VSOL + (m % 300) * 1_000_000 + round * 11;
