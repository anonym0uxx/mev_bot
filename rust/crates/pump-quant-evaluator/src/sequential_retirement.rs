...
//! (operator §49 sizing-vs-edge, §54 edge-decay trend, §56.11 learning
//! horizon).
//!
...
//! Retirement is lane-scoped — the search never retires (operator §2 as
...
//! integer-only (operator §22): all money is `i128` lamports, no floats, no
//! logs, no RNG.
//!
//! Learning-horizon gate (operator §56.11): paper/shadow lanes get a
...
///
/// All fields are integer lamport-scale quantities (operator §22). Cloneable
/// so a supervisor can hold one config per lane.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct RetirementConfig {
    /// Lane this detector is scoped to (operator §48 — lanes never blend).
    pub lane: Lane,
    ...
    /// Minimum reconciled sample before RETIRE may bind (operator §56.11
    /// learning horizon). RETIRE is impossible strictly before this count.
    pub min_samples: u32,
}

/// Sequential verdict for the lane (operator §49). Two-valued as required:
...
///
/// Responsibility (operator §49, §54, §56.11): fold the one-sided CUSUM
