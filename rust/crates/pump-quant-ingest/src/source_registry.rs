...
///
/// - empty input → `None`;
/// - exactly one distinct label → that label preserved;
/// - two or more distinct labels → [`MixLabel::DualOrMultiFeedRecorded`]
///   (feed disagreement is preserved as a multi-feed label, never silently
///   collapsed to one provider's interpretation).
