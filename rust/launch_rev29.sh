#!/usr/bin/env bash
# ════════════════════════════════════════════════════════════════════
# Rev-29 Launch Script — LaserStream gRPC PRIMARY + Helius WS FALLBACK
# ════════════════════════════════════════════════════════════════════
#
# Architecture:
...
echo "[launch] Starting watchdog with Rev-29 daemon args..."
...
./target/release/pq-watchdog.exe \
    ...
    --backoff-cap-secs 30 \
    --daemon-args "--live --wallet-address $WALLET_ADDRESS --junction-cap 8192 --commitment processed --status-every-ticks 50 --brain-snapshot-every-ticks 5000 --tape-every-ticks 200 --refiner-every-ticks 72000 --strategy-label rev29-live" \
    > data/watchdog_rev29.log 2>&1 &
...
if grep -q "LaserStream spawned" data/watchdog_rev29.log 2>/dev/null || \
...
if grep -q "Helius WS" data/watchdog_rev29.log 2>/dev/null || \
...
echo ""
echo "[launch] Rev-29 daemon is live. Monitor with:"
